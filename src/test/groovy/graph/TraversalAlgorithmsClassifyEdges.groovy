...
class TraversalAlgorithmsClassifyEdges extends Specification {
    ...
    def 'classify-edges directed graph with all edge types'() {
        ...
        def firstTree = []
        Map result = classifyEdgesTraversal(graph, [root:'A', componentRoot:'A', colors:[:]]) { from, to, type ->
            Map map = [:]
            map.from = from
            map.to = to
            map.type = type
        ...
        result.remove('forrest')
        result == [root:'G', componentRoot:'F', colors:[A:BLACK, B:BLACK, C:BLACK, D:BLACK, E:BLACK, F:BLACK, G:BLACK], state:CONTINUE]
